...
def load_data():
    try:
        ...
        df1_part4 = pd.read_csv("detik-4.csv", encoding='utf-8')
        df1_part5 = pd.read_csv("detik-5.csv", encoding='utf-8')
        df1_part6 = pd.read_csv("detik-6.csv", encoding='utf-8')
        df1_part7 = pd.read_csv("detik-7.csv", encoding='utf-8')
        df1_part8 = pd.read_csv("detik-8.csv", encoding='utf-8')
        ...
        def categorize_kanal(kanalid, page_url=None):
            ...
            if page_url == "https://www.detik.com/":
                return "WP"
        ...
        if 'Kanal ID' in df2.columns:
            ...
            def categorize_kanal_df2(kanalid):
                ...
                if kanalid_str == "2":
                    return "WP"
                elif kanalid_str.startswith("2-605"):
                    return "News"
                elif kanalid_str.startswith("2-69"):
                    return "Sport"
                elif kanalid_str.startswith("2-207"):
...
def auto_download_csv(filtered_df):
    ...
    if not filtered_df.empty:
        ...
        if contact_columns:
            ...
            # Fix phone number format - IMPROVED VERSION
            if 'phone_number' in contact_df.columns:
                def clean_phone_number(x):
                    ...
                    # If it's already a clean number (no decimal), return as is
                    if '.' not in phone_str and '-' not in phone_str:
                        return phone_str
                    
                    # If it contains hyphens, keep as is (it's already formatted)
                    if '-' in phone_str:
                        return phone_str
                    
                    # If it's a float-like string (ends with .0), remove the decimal part
                    try:
                        # Try to convert to float first, then to int to remove .0
                ...
                contact_df['phone_number'] = contact_df['phone_number'].apply(clean_phone_number)
